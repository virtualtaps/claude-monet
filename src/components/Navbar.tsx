...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <div className="hidden md:block bg-[#0d0d0d] border-b border-[#1a1a1a] py-2">
      ...
      <motion.nav
        ...
        className={`sticky top-0 z-50 transition-all duration-500 ${
          scrolled
            ? "bg-[#0a0a0a]/95 backdrop-blur-lg shadow-lg shadow-black/20"
        ...
        <div className="container mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16 sm:h-20">
            ...
            <div className="hidden lg:flex items-center gap-8">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className="text-sm font-medium text-foreground/80 hover:text-gold transition-colors duration-300 animated-underline"
                >
                  {link.name}
                </Link>
              ))}
            ...
            <div className="hidden lg:flex items-center gap-4">
              <Button
                className="bg-gold hover:bg-gold-light text-black font-semibold px-6 py-5 rounded-none transition-all duration-300 hover:shadow-lg hover:shadow-gold/20"
              >
                Book Now
              </Button>
            </div>
            ...
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
              <SheetTrigger asChild className="lg:hidden">
                <Button 
                  variant="ghost" 
                  size="icon" 
                  className="text-gold hover:bg-gold/10 h-10 w-10"
                  aria-label="Open navigation menu"
                >
                  <Menu className="w-6 h-6" />
                </Button>
              </SheetTrigger>
              <SheetContent side="right" className="bg-[#0a0a0a] border-l border-[#1a1a1a] w-full sm:w-80 p-0">
                ...
                <div className="flex flex-col h-full">
                      ...
                      {navLinks.map((link, index) => (
                        ...
                        >
                          <Link
                            ...
                            className="block py-4 px-4 text-base font-medium text-foreground/90 hover:text-gold hover:bg-[#1a1a1a] transition-all duration-300 rounded-lg active:bg-[#1a1a1a]"
                          >
                            {link.name}
                          </Link>
                        </motion.div>
                  ...
                  {/* Footer Section */}
                  <div className="mt-auto border-t border-[#1a1a1a] px-6 py-6 space-y-4">
                    ...
                    <Button 
                      className="w-full bg-gold hover:bg-gold-light text-black font-semibold py-6 rounded-none transition-all duration-300"
                      onClick={() => setIsOpen(false)}
                    >
                      Book Now
                    </Button>
